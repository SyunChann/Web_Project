...
function FearlessPanel({ series }: { series: ReturnType<typeof fearlessSeries> }) {
  return <TablePanel title="Fearless Draft 시리즈" description="같은 시리즈에서 팀별 챔피언 재사용과 챔피언 풀 확장을 보여 줍니다. 재사용이 적을수록 넓은 풀을 활용한 것입니다."><table><thead><tr><th>날짜·패치</th><th>매치업</th><th>세트</th><th>팀별 고유 픽</th><th>재사용</th></tr></thead><tbody>{series.slice(0, 20).map((row) => <tr key={row.id}><td className="whitespace-nowrap">{row.date}<br /><span className="text-xs text-[#7b8798]">{row.patch}</span></td><td>{row.teams.map((team) => team.name).join(" vs ")}</td><td>{row.games.length}</td><td>{row.teams.map((team) => <div key={team.name} className="whitespace-nowrap">{team.name}: <strong>{team.unique}</strong></div>)}</td><td>{row.teams.map((team) => <div key={team.name} className={`whitespace-nowrap ${team.repeats ? "text-[#c43c46]" : "text-[#087443]"}`}>{team.name}: <strong>{team.repeats}</strong></div>)}</td></tr>)}</tbody></table>{series.length === 0 ? <p className="p-5 text-sm font-bold text-[#718096]">선택한 조건에 2세트 이상 진행된 시리즈가 없습니다.</p> : null}</TablePanel>;
}
